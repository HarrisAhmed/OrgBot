...
class Folder(commands.Cog):
    ...
    async def create(self, inter, name, emoji:discord.Emoji):
        ...
        v.inter = inter
        await inter.send(embed=discord.Embed(title="Folder Creation Process", description=f"**Folder Name: **{name}\n\n**Selected Cards:**\nNone", color=0xfcb8b8), view=v)
        await v.wait()
        if v.value:
            await self.bot.folder(inter.author.id, name=name, add=True, ids=v.ids)
        else:
            await inter.edit_original_message(embed=discord.Embed(description="<:HN_X:1035085573104345098> You cancelled this process!", color=0xfcb8b8), view=None)
    ...
    async def show(self, inter, name):
        ...
        for i in range(0, len(ids), 5):
            emb = discord.Embed(title=f"{name} | Folder", color=0xfcb8b8)
            nl = ids[i:i+5]
            for id in nl:
                for c in inv:
                    if id == c[0].split(" ")[0]:
    ...
    async def name_auto(self, inter, input):
        r = await self.bot.folder(inter.author.id, get=True)
        return [name.split(" ")[1] for name in r.keys() if input in name]
    ...
    async def list(self, inter):
        ...
        for i in range(0, len(r1), 5):
            emb = discord.Embed(title=f"{inter.author.display_name}'s folders:", color=0xfcb8b8)
            nl = r1[i:i+5]
    ...
    async def delete(self, inter, folder):
        ...
        v = ConfirmView()
        v.inter = inter
        await inter.send(embed=discord.Embed(description=f"Are you sure you want to delete this folder? ({folder})", color=0xfcb8b8), view=v)
        await v.wait()
        if not v.value:
            await inter.edit_original_message(embed=discord.Embed(description="<:HN_X:1035085573104345098> You cancelled this process!", color=0xfcb8b8), view=None)
        else:
            await self.bot.folder(inter.author.id, name=folder, delete=True)
            await inter.edit_original_message(embed=discord.Embed(description=f"<:HN_Checkmark:1035085306346606602> You successfully deleted the folder! ({folder})", color=0xfcb8b8), view=None)
